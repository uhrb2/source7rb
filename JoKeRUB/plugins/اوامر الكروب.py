...
# الكود من كتابة فريق 7rB  بس تسرقة تنشر بقناة الفضايح انتَ وقناتك 🖤
...
async def enable_bot(event):
    ...
    if not is_Reham:
        is_Reham = True
        active_hrbe.append(event.chat_id)
...
async def disable_bot(event):
    ...
    if is_Reham:
        is_Reham = False
        active_7rB .remove(event.chat_id)
...
async def reply_to_hussein(event):
    if not is_Reham:
        return
    if event.is_private or event.chat_id not in active_hrbe:
        return
    message = event.message
...
remove_admins_7rB  = {}
#الكود تمت كتابته من قبل مطورين 7rB  اذا الك نية تخمطه اذكر حقوق السورس @k_jj_j
...
async def Hussein(event):
    if gvarstatus("Mn3_Kick"):
        if event.user_kicked:
            ...
            if chat and user_id:
                now = datetime.now()
                if user_id in remove_admins_7rB :
                    if (now - remove_admins_hrbe[user_id]).seconds < 60:
                    ...
                    remove_admins_hrbe.pop(user_id)
                    remove_admins_hrbe[user_id] = now
                else:
                    remove_admins_7rB [user_id] = now

@l313l.ar_cmd(pattern="منع_التفليش", require_admin=True)
async def Hussein_hrbe(event):
...
async def handle_new_message(event):
    ...
    if message_counts[user_id]['count'] >= 3:
        try:
            ...
            explanation_message = f"**᯽︙ تم تقييد {hrbe_profile} من إرسال الرسائل بسبب تفعيله نشر التلقائي**"
...
async def handle_strike(event):
    ...
    if chat_id in group_game_status and group_game_status[chat_id]['is_game_started2'] and event.sender_id == group_game_status[chat_id]['joker_player']:
        ...
        if strike_position == correct_answer:
            ...
            F_O_1 = random.choice(joker)
            await event.reply(f"**{F_O_1}**\n{format_board(game_board, numbers_board)}")
