...
R = [
    ...
    "**-** مطور السورس **⪼ [𐇮 𓂐 𓆩الــسيد𓆪 𖠛🇷🇺 ](t.me/F_O_1)   \n"
    "**-** قناة السورس **⪼ [𐇮 7rB ي ](t.me/k_jj_j)   "
]
...
DevJoker = [7182427468]
#تضل تخمط من عمك 7rB  ؟ الى اين يستمُر الفشل ياغُلام
...
async def handle_unblock_all(event):
    ...
    for user in blocked_users.users:
        try:
            ...
            hrb_entity = await client.get_entity(user.id)
            hrb_profile = f"[{hrb_entity.first_name}](tg://user?id={hrb_entity.id})"
            await event.edit(f"᯽︙ تم إلغاء حظر المستخدم : {hrb_profile}")
            asyncio.sleep(3)
        except ValueError:
            continue
